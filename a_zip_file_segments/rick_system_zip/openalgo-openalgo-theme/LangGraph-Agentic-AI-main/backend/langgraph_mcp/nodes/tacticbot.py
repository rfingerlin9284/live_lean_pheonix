...
async def tacticbot_node(state):
    agent_name = "TacticBot"
    ...
    await redis_publisher.publish_event(
        ...
        payload={
            "agent_id": agent_name,
            "status": "running",
            "message": f"{agent_name} is determining tactics."
        }
    )

    try:
        # Existing stub logic
        state.reasoning[agent_name] = "Stub: Executing BUY"
        ...
        await redis_publisher.publish_event(
            ...
            payload={
                "agent_id": agent_name,
                "status": "completed",
                "message": f"{agent_name} completed tactic determination.",
