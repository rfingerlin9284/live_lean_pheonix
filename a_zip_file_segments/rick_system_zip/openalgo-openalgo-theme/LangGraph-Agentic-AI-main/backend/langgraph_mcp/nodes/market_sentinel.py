...
async def market_sentinel_node(state):
    ...
    await redis_publisher.publish_event(
        ...
        payload={
            "agent_id": agent_name,
            "status": "running",
            "message": f"{agent_name} is scanning market."
        }
    )

    try:
        # Existing stub logic
        state.reasoning[agent_name] = "Stub: Market volatility low"
        ...
        await redis_publisher.publish_event(
            ...
            payload={
                "agent_id": agent_name,
                "status": "completed",
                "message": f"{agent_name} completed market scan.",
