...
def process_line(line):
    ...
    try:
        ...
        # Fallback: Print raw line with clear prefix and color
        print(f"{Colors.BG_RED}{Colors.BOLD} RAW DATA (unparsed): {Colors.RESET}{Colors.YELLOW}{line.strip()}{Colors.RESET}")
    except Exception as e:
        # Fallback: Try to extract basic info if possible
        if hasattr(e, 'args') and e.args:
            print(f"{Colors.BG_RED}{Colors.BOLD} Narration Error: {Colors.RESET}{Colors.YELLOW}{str(e)}{Colors.RESET}")
        # Attempt to print basic trade info if present in line
        if 'direction' in line or 'sl' in line or 'pip' in line:
            print(f"{Colors.BG_YELLOW}{Colors.BOLD} BASIC TRADE INFO: {Colors.RESET}{Colors.WHITE}{line.strip()}{Colors.RESET}")
...
def main():
    ...
    while True:
        try:
            ...
            reload_attempts += 1
            print(f"{Colors.BG_YELLOW}{Colors.BOLD} Narration error detected: {str(e)}. Attempting reload...{Colors.RESET}")
            time.sleep(2)
            if reload_attempts <= max_reload_attempts:
                continue  # Try to reload the script
            else:
                print(f"{Colors.BG_RED}{Colors.BOLD} Narration reload failed. Invoking self-repair helper...{Colors.RESET}")
                # Call helper script for self-repair and alerting
                import subprocess
                try:
                    subprocess.run(["python3", "narration_self_repair.py"], check=False)
                except Exception as repair_error:
                    print(f"{Colors.BG_RED}{Colors.BOLD} Self-repair failed: {str(repair_error)}{Colors.RESET}")
                print(f"{Colors.BG_YELLOW}{Colors.BOLD} Fallback: Showing raw trade info only. System remains connected.{Colors.RESET}")
                reload_attempts = 0
