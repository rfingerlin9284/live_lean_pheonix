...
def main():
    ...
    for instrument in instruments:
        try:
            ...
            if sma20 > sma50 and 0 < momentum < 0.15:
                near_miss = f'momentum {momentum:.3f}% < threshold 0.15%'
    ...
    if active_signals:
        ...
        for s in active_signals:
            ...
            # Format momentum with parentheses if negative
            momentum_val = s['momentum']
            if momentum_val < 0:
                momentum_display = f"({momentum_val:.3f}%)"
            else:
                momentum_display = f"+{momentum_val:.3f}%"
    ...
    if near_misses:
        ...
        for nm in near_misses:
            ...
            # Format momentum with parentheses if negative
            momentum_val = nm['momentum']
            if momentum_val < 0:
                momentum_display = f"({momentum_val:.3f}%)"
            else:
                momentum_display = f"{momentum_val:.3f}%"
            
            # Format reason text to use parentheses for negatives
            reason_text = nm['reason']
            if 'momentum -' in reason_text or momentum_val < 0:
                reason_text = reason_text.replace('momentum -', 'momentum (').replace('%', '%)')
                if '> -' in reason_text:
                    reason_text = reason_text.replace('> -', '> ')
                if '< -' in reason_text:
                    reason_text = reason_text.replace('< -', '< ')
            
            print(f"  Momentum: {momentum_display} ({reason_text})")
