...
class AutoDiagnosticMonitor:
    ...
    def _check_auth_tokens(self) -> Dict:
        ...
        # Check Coinbase credentials (check .env first, then legacy .env.coinbase_advanced)
        env_file = '.env' if os.path.exists('.env') else '.env.coinbase_advanced'
        if os.path.exists(env_file):
            with open(env_file, 'r') as f:
                content = f.read()
                has_key = 'COINBASE_LIVE_API_KEY' in content or 'CDP_API_KEY_NAME' in content
                has_secret = 'COINBASE_LIVE_API_SECRET' in content or 'CDP_PRIVATE_KEY' in content
                status['details']['coinbase'] = "✅ CONFIGURED" if (has_key and has_secret) else "⚠️ INCOMPLETE"
        else:
            status['details']['coinbase'] = "❌ NOT FOUND"
            status['status'] = "FAIL"
        
        # Check OANDA credentials (check .env first, then legacy .env.oanda_only)
        oanda_env = '.env' if os.path.exists('.env') else '.env.oanda_only'
        if os.path.exists(oanda_env):
            with open(oanda_env, 'r') as f:
                content = f.read()
