...
class AllocationManager:
    ...
    def adjust_for_min_notional(self, adjusted: float, market_conditions: Dict[str, float]) -> float:
        """
        Ensure the adjusted position size meets the dynamic minimum notional requirement.
        Args:
            adjusted: The calculated position size.
            market_conditions: A dictionary containing market metrics (e.g., volatility).
        Returns:
            Adjusted position size that meets the minimum notional requirement.
        """
        try:
            from foundation.rick_charter import RickCharter
            min_notional = RickCharter.dynamic_min_notional(market_conditions)
            if adjusted < min_notional:
                logger.warning(f"Adjusted size {adjusted:.2f} below dynamic min notional {min_notional:.2f}. Scaling up.")
                adjusted = min_notional
        except Exception as e:
            logger.error(f"Error adjusting for min notional: {e}")
        return adjusted

    def calculate_size(self, strategy_name: str, base_notional: float, portfolio_state: Optional[Dict[str, Any]] = None, entry: Optional[float] = None, sl: Optional[float] = None, market_data: Optional[Dict[str, Any]] = None, market_conditions: Optional[Dict[str, float]] = None) -> float:
        ...
        # Ensure adjusted size respects dynamic charter min_size
        if market_conditions:
            adjusted = self.adjust_for_min_notional(adjusted, market_conditions)
