...
class OandaConnector:
    ...
    def _load_credentials(self):
        ...
        # Search for .env in several common locations to be resilient across different repo layouts.
        candidate_paths = [
            os.path.join(os.path.dirname(__file__), '..', '.env'),  # oanda/.env
            os.path.join(os.path.dirname(__file__), '..', '..', '.env'),  # repo root .env
            os.path.join(os.getcwd(), '.env'),  # current working dir .env
            os.path.expanduser('~/.env')  # user home .env
        ]
        found = None
        ...
        if found:
            self.logger.info(f"Loading credentials from .env at {found}")
            ...
            self.logger.debug("No .env file found in candidate locations; relying on environment variables or defaults")
