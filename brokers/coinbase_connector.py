    ...
    def place_oco_order(self, product_id: str, entry_price: float, stop_loss: float, 
        ...
        try:
            if not execution_gate.can_place_order():
                ...
                try:
                    log_narration(event_type='EXECUTION_DISABLED', details={'instrument': product_id, 'reason': 'EXECUTION_DISABLED_OR_BREAKER'}, symbol=product_id, venue='coinbase')
                except Exception:
                    pass
                return {"success": False, "error": "EXECUTION_DISABLED_OR_BREAKER", "broker": "Coinbase", "environment": self.environment}
        except Exception as e:
            self.logger.warning(f'Execution gate check failed: {e} - Blocking order for safety')
            return {"success": False, "error": "EXECUTION_GATE_CHECK_FAILED", "broker": "Coinbase", "environment": self.environment}
