...
def bracket_for_requirements(direction, entry, units, rr=3.2, min_profit_usd: float = None):
    # Default risk distance ~20 pips for non-JPY USD-quoted majors (pip=0.0001)
    pip = 0.0001
    risk_dist = 20 * pip
    # Profit distance meets both RR * risk and >= $35 projected USD
    min_expected = min_profit_usd or (getattr(RickCharter, 'MIN_EXPECTED_PNL_USD', 0.0) if RickCharter else 0.0)
    min_dist_for_100 = (min_expected / float(units))
    ...
    while True:
        ...
        for ev in events:
            ...
            try:
                ...
                sl, tp = bracket_for_requirements(consensus, mid, units, rr=3.2)
