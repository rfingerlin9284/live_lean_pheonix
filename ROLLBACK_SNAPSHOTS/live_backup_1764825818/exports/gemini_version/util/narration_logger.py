#!/usr/bin/env python3
"""
Minimal narration logger for unit testing and CLI runs.
Provides minimal functions used by the trading engine to record events.
"""
import json
import time
from datetime import datetime

def log_narration(event_type: str, details: dict, symbol: str = 'SYSTEM', venue: str = 'oanda'):
    record = {
...
def log_pnl(symbol: str, pnl: float, details: dict = None):
    record = {
        'timestamp': datetime.utcnow().isoformat() + 'Z',
        'event_type': 'PNL',
        'symbol': symbol,
...
NARRATION_FILE = Path(os.getenv('NARRATION_FILE_OVERRIDE', str(PROJECT_ROOT / "narration.jsonl")))  # Root level for dashboard access
...
def log_narration(
    ...
    try:
        ...
        # Allow overriding the narration file at runtime with an environment variable
        target_file = Path(os.getenv('NARRATION_FILE_OVERRIDE', str(NARRATION_FILE)))
...
from util.confidence import format_confidence as format_conf

def format_confidence(confidence: float) -> str:
    """Wrapper to reuse util.confidence.format_confidence for backward compat"""
    return format_conf(confidence)
