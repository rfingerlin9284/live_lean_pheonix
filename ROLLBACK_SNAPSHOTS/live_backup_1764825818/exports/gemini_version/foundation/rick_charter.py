...
class RickCharter:
    ...
    MIN_EXPECTED_PNL_USD = 35.0  # Gross PnL at TP must be >= $35
    ...
    def validate(cls, test_key: Optional[str] = None) -> bool:
        ...
        try:
            ...
            assert cls.MIN_NOTIONAL_USD == 10000, "Notional minimum error"
            assert cls.MIN_RISK_REWARD_RATIO == 3.2, "Risk reward error"
            assert cls.MIN_EXPECTED_PNL_USD == 35.0, "Expected PnL minimum error"
            ...
            assert cls.validate_notional(10000) == True, "10k should be valid"
            assert cls.validate_notional(9999) == False, "9999 should be invalid"
    ...
    @classmethod
    def dynamic_min_expected_pnl(cls, market_conditions: Dict[str, float]) -> float:
        """
        Dynamically adjust the minimum expected PnL based on market conditions.
        Args:
            market_conditions: A dictionary containing market metrics (e.g., volatility, spread).
        Returns:
            Adjusted minimum expected PnL.
        """
        base_pnl = cls.MIN_EXPECTED_PNL_USD
        ...
        if volatility > 1.5:  # High volatility
            base_pnl *= 0.8  # Reduce threshold by 20%
        elif volatility < 0.8:  # Low volatility
            base_pnl *= 1.2  # Increase threshold by 20%

        if spread > 20_000:  # High spread
            base_pnl *= 1.1  # Increase threshold by 10%

        return max(base_pnl, 35.0)  # Ensure a minimum threshold of $35

    @classmethod
    def dynamic_min_notional(cls, market_conditions: Dict[str, float]) -> float:
        """
        Dynamically adjust the minimum notional value based on market conditions.
        Args:
            market_conditions: A dictionary containing market metrics (e.g., volatility, spread).
        Returns:
            Adjusted minimum notional value.
        """
        base_notional = cls.MIN_NOTIONAL_USD
        volatility = market_conditions.get("volatility", 1.0)  # Default to neutral volatility

        # Example adjustment logic
        if volatility > 1.5:  # High volatility
            base_notional *= 1.2  # Increase threshold by 20%
        elif volatility < 0.8:  # Low volatility
            base_notional *= 0.8  # Reduce threshold by 20%

        return max(base_notional, 10_000)  # Ensure a minimum threshold of $10,000
