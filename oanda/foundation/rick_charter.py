...
class RickCharter:
    ...
    def validate(cls, test_key: Optional[str] = None) -> bool:
        ...
        try:
            ...
            # Support both Balanced (10k/35) and Institutional (15k/100) profiles for compatibility
            assert cls.MIN_NOTIONAL_USD in (10000, 15000), "Notional minimum error"
            assert cls.MIN_RISK_REWARD_RATIO == 3.2, "Risk reward error"
            assert cls.MIN_EXPECTED_PNL_USD in (35.0, 100.0), "Expected PnL minimum error"
            ...
            # Ensure validation corresponds to the declared minimum notional
            min_n = cls.MIN_NOTIONAL_USD
            assert cls.validate_notional(min_n) == True, f"{min_n} should be valid"
            assert cls.validate_notional(min_n - 1) == False, f"{min_n-1} should be invalid"
