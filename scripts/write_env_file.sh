...
# Script to write a canonical environment template to `.env` (with backup + lock)
# Usage: ./scripts/write_env_file.sh [target_env_file]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --dry-run)
      ...
      ;;
    --help|-h)
      echo "Usage: $0 [TARGET_ENV] [--dry-run] [--no-lock]"
      exit 0
      ;;
    --no-lock)
      NO_LOCK=true
...
echo "Writing new environment file to ${TARGET_ENV}..."
if [ "$DRY_RUN" = true ]; then
  echo "DRY RUN: would write template to ${TARGET_ENV}."
...
if [ "$DRY_RUN" = true ]; then
  ...
  echo "Done. ${TARGET_ENV} updated and locked."
fi
