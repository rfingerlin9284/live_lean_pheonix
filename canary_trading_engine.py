...
class CanaryTradingEngine(CharterCompliantGhostEngine):
    ...
    def __init__(self, pin: int = 841921):
        ...
        print("=" * 80)
        print("🐤 CANARY MODE (ALIAS: PAPER PRACTICE) - Quick Validation Trading (45 minutes)")
        print("=" * 80)
        print("This CANARY session will:")
        print("  ✅ Enforce ALL Charter rules ($15K notional, 3.2 RR, 6h max hold)")
        print("  ✅ Uses PAPER/PRACTICE credentials (CANARY is just an alias)")
    ...
    # Example engine-side adapter usage (SAFE, COMMENTED) ----------------------
    # To integrate research strategies into the engine without modifying execution
    # logic, use the research_strategies/engine_adapter.py and call:
    #
    # from research_strategies.engine_adapter import get_signals_for_symbol
    # df_ohlcv = self.get_ohlcv_for_symbol('BTCUSD')  # example method to grab OHLCV
    # signals_df = get_signals_for_symbol('BTCUSD', df_ohlcv)
    # if not signals_df.empty:
    #     # Wrap signals into current engine's order placement workflow (charter gating)
    #     top_signal = signals_df.iloc[0]
    #     if top_signal['confidence'] >= 0.65 and not self.session_breaker.is_breaker_active:
    #         # pass the signal into existing order creation pipeline
    #         create_order_from_signal(top_signal)
    #
    # This is intentionally commented and provided only as safe guidance.
